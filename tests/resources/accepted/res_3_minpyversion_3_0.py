...
class DainDataset(data.Dataset):
    def __init__(
        ...
        for i in range(0, len(my_list) - 1):
            if diffScenes > -1:
                skip_interpolation = psnr.IsDiffScenes(
                    my_list[i], my_list[i + 1], diffScenes
                )
    ...
    def Convert(self, a):
        return torch.FloatTensor(
            a.transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)
        )
    ...
    def __getitem__(self, index):
        ...
        my_combo["original"] = numpy.array(
            PIL.Image.open(self.combos[index]["f1"]).convert(self.frameFormat)
        )
