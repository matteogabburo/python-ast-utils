...
class DAIN_slowmotion(torch.nn.Module):
    ...
    def forward(self, input_0, input_2, padding):
        ...
        with torch.cuda.stream(s1):
            ...
            if self.useAnimationMethod == 1 or self.useAnimationMethod == 2:
                ...
                temp = self.depthNet(
                    torch.cat(
                        (cur_filter_input[:, :3, ...], cur_filter_input[:, 3:, ...]),
                        dim=0,
                    )
                )
        ...
        for temp_0, temp_1, timeoffset in zip(
            ...
            cur_output_temp = cur_output_temp[
    ...
    def forward_flownets(self, model, input, time_offsets=None):
        ...
        temps = [
            nn.Upsample(scale_factor=4, mode="bilinear", align_corners=True)(temp)
            for temp in temps
    ...
    def FilterInterpolate(
        filterModule, ref0, ref2, offset, filter, filter_size2, time_offset
    ):
    ...
    def conv_relu_unpool(
        ...
        layers = nn.Sequential(
            *[
                nn.Upsample(
                    scale_factor=unpooling_factor, mode="bilinear", align_corners=True
                ),
                nn.Conv2d(input_filter, output_filter, kernel_size, 1, padding),
                nn.ReLU(inplace=True),
                # nn.BatchNorm2d(output_filter),
