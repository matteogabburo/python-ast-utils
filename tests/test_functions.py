...
def _get_resources(folder_path):
    ...
    ret = [(os.path.join(folder_path, filename), _get_min_py_version(filename)) for filename in os.listdir(folder_path)]
    return ret
    ...
    for fname, min_pyversion in resources:
        for mode in modes:
            for type_comment in type_comments:
                for feature_version in [(major, minor) for major in range(min_pyversion[0], MAX_PY_VERSION[0]+1) for minor in range(min_pyversion[1], MAX_PY_VERSION[1]+1)]:
                    tests.append((fname, mode, type_comment, feature_version))
    return tests

@pytest.mark.parametrize(
    "fname,mode,type_comments,feature_version",
    _ast_parse_parameters(RESOURCES_PATH),
)
def test_ast_parse(
    fname,
    mode,
    ...
    astutils.ast_parse(fname, mode=mode, type_comments=type_comments, feature_version=feature_version)
...
def test_ast_parse_from_string(
    fname,
    mode,
    type_comments,
    feature_version
):
    astutils.ast_parse_from_string(_read(fname), mode=mode, type_comments=type_comments, feature_version=feature_version)
...
def test_ast_unparse(
    ...
    ast_tree = astutils.ast_parse(fname, mode=mode, type_comments=type_comments, feature_version=feature_version)
...
def test_ast2dict_dict2ast(
    fname,
    mode,
    type_comments,
    feature_version
):
    ast_tree1 = astutils.ast_parse(fname, mode=mode, type_comments=type_comments, feature_version=feature_version)
...
def test_ast2heap_heap2tokens(
    ...
    ast_heap = astutils.ast2heap(ast_tree, source=sourcecode)
    source_code_1 = "".join([tokens[0] for tokens in astutils.heap2tokens(ast_heap)])
