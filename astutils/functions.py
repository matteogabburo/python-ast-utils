...
def heap2tokens(heap: list) -> list:
    ...
    def _dfs_build(root, heap, parent):
        ...
        if root[HEAP_CODE]:
            root[HEAP_TOKENS] = _code2tokens(root[HEAP_CODE], root[HEAP_ID], root[HEAP_TYPE])
        else:
            root[HEAP_TOKENS] = _code2tokens(root[HEAP_PLACEHOLDER], root[HEAP_ID], root[HEAP_TYPE])
        ...
        return [
            (re.sub(NUMBER_PLACEHOLDER.format(Number), __replace, token), node_id, node_type)
