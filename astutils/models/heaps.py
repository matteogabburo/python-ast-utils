...
class TokenizedTuple:

    def __init__(self, tok, node_pos=None, node_type=None, is_head=True):
...
class CodeSegment:
    ...
    def _unmask_numbers(code):
        return code.replace(NUMBER_PLACEHOLDER_BEG, "").replace(
    ...
    def replace_next(self, sep, to_replace, node_id, node_type):
        ...
        if to_replace.code_segment in self.code_segment:
            ...
            if to_replace.code_segment in code_l[-1]:
                ...
                self.code_segment = self.code_segment.replace(
                    to_replace.code_segment, CHILD_PLACEHOLDER.format(node_id), 1
                )

            self._segments_map[
                CHILD_PLACEHOLDER.format(node_id)
            ] = to_replace.code_segment
    ...
    def resolve_tuple(self, node_id=None, node_type=None, heap=None):
        ...
        def _build_tuple(segment):
            return TokenizedTuple(segment, node_pos=node_id, node_type=node_type, is_head=True)
        ...
        if self.segments is not None:
            for segment in self.segments:
                ...
                if segment in self._segments_map:
                    ...
                    resolved.append(_build_tuple(self._unmask_numbers(segment)))
    ...
    def resolve_tokenized_tuple(self, heap, node_id=None, node_type=None):
        ...
        def _build_tuple(segment, is_head):

            return TokenizedTuple(segment, node_pos=node_id, node_type=node_type, is_head=is_head)
        ...
        for segment in self.tokenized_segments:
            ...
            if isinstance(segment, str) and segment in self._positions_map:
                ...
                if heap:
                    ...
                    if isinstance(accumulator, list):
                        ...
                        raise NotImplementedError(
                            "Some error has occured, check the code"
                        )
                else:
                    raise NotImplementedError("Initialize the heap parameter")
                ...
                if isinstance(segment, str):
                    resolved.append(_build_tuple(segment, head))

                elif isinstance(segment, list):
                    head = True
                    for sub_seq in segment:
                        resolved.append(_build_tuple(sub_seq, head))
                        head = False
    ...
    def tokenize(self, regex_pattern, fn_tokenize, *args, **kwargs):
        ...
        for segment in self.segments:
            ...
            if segment not in self._segments_map:
                ...
                self.tokenized_segments.append(
                    fn_tokenize(self._unmask_numbers(segment), *args, **kwargs)
                )
...
class HeapNode:
    def __init__(
        self, node_id, parent, node_type, node_value=None, code=None, children=None
    ):
        ...
        self.abstract_depth = (
            0 if self.parent is None else self.parent.abstract_depth + 1
        )
        self.abstract_walk = (
            [node_id] if self.parent is None else self.parent.abstract_walk + [node_id]
        )
        self.depth = None
        self.walk = None
    ...
    def update_walk(self):
        ...
        if not self.is_abstract():
            self.walk = (
                ...
                else self.parent.walk + [self.node_id]
            )
        else:
    ...
    def add_code_segment(self, code_segment):
        self.code = (
            CodeSegment(code_segment) if isinstance(code_segment, str) else code_segment
        )
    ...
    def _update_code(self, node, to_replace, node_id, node_type):
        ...
        if not node.is_root():
            ...
            if not node.is_abstract() and not node.parent.is_abstract():
                success = node.parent.code.replace_next(
                    CHILD_PLACEHOLDER_END, to_replace, node_id, node_type
                )

            elif not node.is_abstract() and node.parent.is_abstract():
                success = node.parent.placeholder.replace_next(
                    CHILD_PLACEHOLDER_END, to_replace, node_id, node_type
                )
...
class Heap:
    ...
    def is_tokenized(self):
        return self.get_root().is_tokenized()
    ...
    def _build_heap(self, tree: ast.AST, source, field, parent):
        ...
        if isinstance(tree, ast.AST):
            ...
            if self._have_source():
                heap_node.add_code_segment(
                    get_source_segment(self._get_source_map(), tree, padded=self.padded)
                )
    ...
    def _check_if_possible_to_return_values(self):
        if (
            not self._is_tree_empty()
            and not self._only_root()
            and self._is_code_embedded()
        ):
            return True
            ...
            raise Exception(
                "Source code was not passed as parameter during the Heap construction, this method is unavailable"
            )
    ...
    def get_heap_tuples(self):
        ...
        if self._check_if_possible_to_return_values():
            return [t.to_dict() for t in self.get_root().get_tuple(heap=self._get_heap())]
        else:
            return ""
    ...
    def get_heap_tokenized_tuples(self, fn_tokenize, *args, **kwargs):
        ...
        if self._check_if_possible_to_return_values():
            ...
            return [t.to_dict() for t in self.get_root().get_tokenized_tuple(heap=self._get_heap())]
        else:
            return ""
    ...
    def decompose(self, min_size=1, max_size=None, measure="nnodes"):
        ...
        if measure == 'nntokens' and not self.is_tokenized():
            raise Exception("To use 'measure=ntokens' you must call heap.tokenize() first")
    ...
    def gredy_decomposition(
        self, min_size=1, max_size=None, mode="max", measure="nnodes"
    ):
        ...
        def _min_decomposition(node_id, walk=[], banned=[]):
            ...
            if not curr_node.is_abstract() and curr_node.num_tokenized_tokens():
                if (
                    sub_size >= min_size
                    and sub_size <= max_size
                    and node_id not in banned
                ):
                    res += [node_id]
                    banned += walk
        ...
        if mode == "max":
            ...
            raise ValueError(
                "this mode is not supported (chose between 'max' and 'min'"
            )
    ...
    def tokenize(self, fn_tokenize, *args, **kwargs):
        ...
        for node_id in self._get_heap():
            ...
            self.get_node(node_id).tokenize(
                self._get_child_pattern(), fn_tokenize, *args, **kwargs
            )

        # update tokens counts
        for node_id in self._get_heap():
