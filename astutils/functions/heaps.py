...
def ast2heap(
    ...
    return Heap(
        ...
        not_considered_leaves=not_considered_leaves,
        padded=padded,
    )
...
def greedy_decompose(
    heap: Heap, min_size=1, max_size=None, mode="max", measure="ntokens"
) -> List[Heap]:
    ...
    return heap.gredy_decomposition(
        min_size=min_size, max_size=max_size, mode=mode, measure=measure
    )
...
def heap2code(heap: Heap) -> str:
    ...
    return "".join([t["tok"] for t in heap2tokens(heap)])
...
def heap2tokenized_tokens(
    heap: Heap, fn_tokenize: Callable[[str], List[str]], *args, **kwargs
    ...
    return heap.get_heap_tokenized_tuples(fn_tokenize=fn_tokenize, *args, **kwargs)
